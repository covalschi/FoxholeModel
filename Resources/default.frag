...
struct Parameters
{
    ...
    float RoughnessMax;
    float EmissiveMult;
    vec4 ColorShift; // rgb target, a strength
    float MudLevel;
    float SnowLevel;
    vec3 MudTint;
    vec3 SnowTint;
    sampler2D MudMask;
    sampler2D SnowMask;
    float MudMaskStrength;
    float MudMaskTightness;
    float SnowMaskStrength;
    float SnowMaskTightness;
    float DamageLevel;
    vec3 DamageTint;
};
...
void main()
{
    if (uDebugSolidColor)
    {
        FragColor = vec4(pow(uDebugColor.rgb, vec3(1.0 / 2.2)), uDebugColor.a * uOpacity);
        return;
    }
    ...
    else
    {
        ...
        if (uParameters.HasSpecularMask)
        {
            specSample = SamplerToVector(uParameters.SpecularMasks[layer].Sampler).rgb;
        }
        ...
        if (uParameters.HasColorShift)
        {
            float colorMask = uParameters.HasSpecularMask ? specSample.b : 1.0;
            float strength = clamp(colorMask * uParameters.ColorShift.a, 0.0, 1.0);
            result = mix(result, uParameters.ColorShift.rgb, strength);
        }

        if (uParameters.MudLevel > 0.0)
        {
            float mudMask = clamp(-normals.y, 0.0, 1.0);
            if (uParameters.HasMudMask)
            {
                float maskSample = texture(uParameters.MudMask, fTexCoords).r;
                float tightness = max(uParameters.MudMaskTightness, 0.0001);
                maskSample = pow(clamp(maskSample, 0.0, 1.0), tightness);
                mudMask = maskSample * uParameters.MudMaskStrength;
            }
            else if (uParameters.HasSpecularMask)
                mudMask *= clamp(specSample.r, 0.0, 1.0);
            mudMask = clamp(mudMask * uParameters.MudLevel, 0.0, 1.0);
            vec3 mudTarget = mix(result, uParameters.MudTint, 0.6);
            result = mix(result, mudTarget, mudMask);
        }

        if (uParameters.SnowLevel > 0.0)
        {
            float snowMask = clamp(normals.y, 0.0, 1.0);
            if (uParameters.HasSnowMask)
            {
                float maskSample = texture(uParameters.SnowMask, fTexCoords).r;
                float tightness = max(uParameters.SnowMaskTightness, 0.0001);
                maskSample = pow(clamp(maskSample, 0.0, 1.0), tightness);
                snowMask = maskSample * uParameters.SnowMaskStrength;
            }
            else if (uParameters.HasSpecularMask)
                snowMask *= clamp(1.0 - specSample.g, 0.0, 1.0);
            snowMask = clamp(snowMask * uParameters.SnowLevel, 0.0, 1.0);
            result = mix(result, uParameters.SnowTint, snowMask);
        }

        if (uParameters.DamageLevel > 0.0)
        {
            result = mix(result, uParameters.DamageTint, clamp(uParameters.DamageLevel, 0.0, 1.0));
        }
