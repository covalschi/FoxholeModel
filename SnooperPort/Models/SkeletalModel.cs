...
public class SkeletalModel : UModel
{
    ...
    public SkeletalModel(USkeletalMesh export, CSkeletalMesh skeletalMesh, Transform transform = null)
        : base(export, skeletalMesh.LODs[LodLevel], FilterMaterials(export.Materials), skeletalMesh.LODs[LodLevel].Verts, skeletalMesh.LODs.Count, transform)
    {
    ...
    private static IReadOnlyList<ResolvedObject> FilterMaterials(ResolvedObject?[] materials)
    {
        if (materials == null || materials.Length == 0)
            return Array.Empty<ResolvedObject>();

        var list = new List<ResolvedObject>(materials.Length);
