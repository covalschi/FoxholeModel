...
public abstract class UModel : IRenderableModel
{
    ...
    public virtual void Render(Shader shader, Texture checker = null, bool outline = false)
    {
        ...
        foreach (var section in Sections)
        {
            ...
            Console.WriteLine($"[draw] model={Name} section={section.MaterialIndex} faces={section.FacesCount} instances={TransformsCount}");
            if (!outline)
            {
                var errBeforeMaterials = GL.GetError();
                if (errBeforeMaterials != ErrorCode.NoError)
                    Console.WriteLine($"[gl] pre-material error: {errBeforeMaterials}");
                if (checker != null)
                {
            ...
            var errBeforeDraw = GL.GetError();
            if (errBeforeDraw != ErrorCode.NoError)
                Console.WriteLine($"[gl] pre-draw error: {errBeforeDraw}");
            Console.WriteLine($"[draw-info] model={Name} section={section.MaterialIndex} faces={section.FacesCount} first={section.FirstFaceIndex} ptr={section.FirstFaceIndexPtr} instances={TransformsCount}");
            ...
            var errAfterDraw = GL.GetError();
            if (errAfterDraw != ErrorCode.NoError)
                Console.WriteLine($"[gl] draw error: {errAfterDraw}");
        }
