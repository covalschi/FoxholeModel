...
public class Options
{
    ...
    public bool TryGetTexture(UTexture o, bool fix, out Texture texture)
    {
        ...
        if (bitmap is not null)
        {
            ...
            if (o is UTexture2D tex2D && tex2D.Name.Contains("ShippingContainer", StringComparison.OrdinalIgnoreCase))
            {
                Console.WriteLine($"[options] decoded texture {tex2D.Name} format={tex2D.Format} srgb={(tex2D.SRGB ? "yes" : "no")} size={texture.Width}x{texture.Height}");
            }
