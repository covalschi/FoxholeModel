...
public class Renderer : IDisposable
{
    ...
    private bool _debugSolidColor;
    private Vector4 _debugColor = new(1f, 0f, 0f, 1f);
    ...
    public void Swap(UMaterialInstance unrealMaterial)
    {
        ...
        HeadlessDispatcher.Invoke(() => model.Materials[section.MaterialIndex].Setup(Options, model.UvCount));
    }
    ...
    public void Render()
    {
        ...
        _shader.SetUniform("uDebugSolidColor", _debugSolidColor);
        _shader.SetUniform("uDebugColor", _debugColor);
        for (int i = 0; i < 5; i++)
            _shader.SetUniform($"bVertexColors[{i}]", false);

        // render model pass
        foreach (var model in Options.Models.Values)
        {
            if (!model.IsVisible) continue;
            model.Render(_shader, null);
        }
        ...
        if (Options.TryGetModel(out var selected) && selected.IsVisible)
        {
            ...
            selected.Render(_outline, null, true);
        }
    ...
    public void SetDebugSolidColor(bool enabled, Vector4? color = null)
    {
        _debugSolidColor = enabled;
        if (color.HasValue)
        {
            _debugColor = color.Value;
        }
    }
    ...
    private void LoadStaticMesh(UStaticMesh original, ENaniteMeshFormat naniteFormat = ENaniteMeshFormat.OnlyNormalLODs)
    {
        ...
        if (Options.TryGetModel(guid, out var model))
        {
            ...
            HeadlessDispatcher.Invoke(model.SetupInstances);
            return;
        }
    ...
    {
        if (!TryLoad("Engine/Content/BasicShapes/Cube.Cube", out UStaticMesh editorCube))
            return;
        ...
        if (Options.TryGetModel(guid, out var model))
        {
            ...
            HeadlessDispatcher.Invoke(() => model.Materials[0].Setup(Options, model.UvCount));
            return;
        }
    ...
    private void OverrideJunoVertexColors(UStaticMesh staticMesh, UGeometryCollection geometryCollection = null)
    {
        ...
        if (geometryCollection?.Materials is not { Length: > 0 })
        {
            ...
            foreach (var indexAsByte in distinctReds)
            {
                ...
                if (!TryLoad(path, out UMaterialInterface unrealMaterial))
                    continue;
    ...
    private void AdditionalWorlds(UObject actor, Matrix4x4 relation, CancellationToken cancellationToken)
    {
        ...
        for (int j = 0; j < additionalWorlds.Length; j++)
            if (TryLoad(additionalWorlds[j].AssetPathName.Text, out UWorld w))
                LoadWorld(cancellationToken, w, transform);
    }

    private static bool TryLoad<T>(string path, out T export) where T : UObject
    {
        var provider = GlobalContext.Provider;
        if (provider == null)
        {
            export = default!;
            return false;
        ...
        return provider.TryLoadPackageObject(path, out export);
    }
